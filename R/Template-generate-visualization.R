...
Template$methods(
    generate_visualization = function(){
        ...
        source(file_structure$translator_file)
        html_code <- knit_expand(file_structure$template_file)

        writeLines(html_code, file_structure$output_file)
    ...
    translate_coffee_to_js = function() {
        if (opts$coffee && file.exists(file_structure$template_coffee_file)){
            ...
            coffee_template <- readLines(file_structure$template_coffee_file, warn = FALSE)
            ...
            suppressMessages(knit(text = replaced_coffee_template, output = file_structure$template_file, quiet = TRUE))

            js_template <- paste(readLines(file_structure$template_file), collapse = "\n")
            replaced_js_template <- replace_delimiter(js_template, coffee_delimiter, js_delimiter, deparse = FALSE)

            writeLines(replaced_js_template, file_structure$template_file)
        }
    },
