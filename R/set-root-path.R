...
#' It should contain the ractives folder. The server should be run from here.
#' @param path path to be used as root
#' @export
set_root_path <- function(path) {
    if (!file.exists(path)) stop("Path doesn't exist: ", path)

    if (!file.exists(file.path(path, .clickme_env$ractives_dir_name))) dir.create(file.path(path, .clickme_env$ractives_dir_name))
