...
test_that("the palette is black when color_group is NULL or it has length 1", {
    palette <- get_palette_param(opts)
    expect_equal(palette, "[\"#000\"]")

    opts$data$.color_group <- "a"
    palette <- get_palette_param(opts)
    expect_equal(palette, "[\"#000\"]")
})

test_that("the palette has as many colors as levels (or unique elements) in color_group", {
    opts$data$.color_group <- c("a", "a", "b", "c", "b")
    palette <- get_palette_param(opts)
...
test_that("the color_domain is calculated from the values of color_group", {
    opts$data$.color_group <- c(1, NA, 3, 5, 4)
