  ...
  <body>
    ...
    (function() {
      ...
      plot = new_plot({
        ...
        data_ranges: {
          x: {{  toJSON(range(opts$data$x, na.rm = TRUE))  }},
          y: {{  toJSON(range(opts$data$y, na.rm = TRUE))  }}
        },
        xlab: {{  toJSON(opts$params$xlab)  }},
        ylab: {{  toJSON(opts$params$ylab)  }},
        title: {{  toJSON(opts$params$title)  }},
        box: {{  toJSON(opts$data$box ) }},
        xlim: {{  toJSON(opts$params$xlim ) }},
        ylim: {{  toJSON(opts$params$ylim ) }},
        categorical_domains: {
          x: {{  toJSON(opts$params$x_categorical_domain )  }},
          y: {{  toJSON(opts$params$y_categorical_domain )  }}
        },
      ...
      plot.tooltip_text = function(d) {
        return {{  get_tooltip_content_points(opts$data, opts$params)  }};
      };
      ...
      if (color_scale.range().length > 1) {
        sidebar.append("text").attr({
          ...
          "font-size": "16px",
          "font-weight": "bold"
        }).text({{  toJSON(opts$params$color_title)  }});
