...
(function() {
  var clip, defs, deselect_color_groups, g_color_group_keys, g_color_title, g_lines, g_points, g_toggle_names, keyuped, line, line_names, lines, mouseout, mouseover, overlay_radius, plot, point_data, point_names, points, search, search_clear, search_input, show_all_colors, sidebar, single_group, tip, toggle_lines_and_points, toggle_names, transform_points;
  ...
  lines = g_lines.append("path").attr({
    ...
    "stroke": function(d) {
      return color_scale(d[0].color_group);
    },
  ...
  clip.selectAll("circle.overlay").data(point_data).enter().append("svg:circle").attr({
    "class": "overlay",
    "cx": function(d) {
      return plot.scales.x(d.x);
    },
  ...
  toggle_lines_and_points = function(category) {
    var categories;
    ...
    g_lines.filter(function(d) {
      return d[0].color_group === category;
    }).classed("hide", function() {
      return !d3.select(this).classed("hide");
    });
    ...
    categories = g_lines.filter(":not(.hide)").data().map(function(d) {
  ...
  deselect_color_groups = function() {
    var categories, visible_category, visible_lines, visible_points;
    visible_points = g_points.filter(":not(.hide)");
    visible_lines = g_lines.filter(":not(.hide)");
    categories = visible_lines.data().map(function(d) {
      return d[0].color_group;
    }).unique();
    if (single_group.text() === "Show one") {
      ...
      g_lines.filter(function(d) {
        return d[0].color_group !== visible_category;
      }).classed("hide", true);
      g_color_group_keys.filter(function(d) {
