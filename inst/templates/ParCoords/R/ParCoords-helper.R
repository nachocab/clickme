...
get_colors_param <- function(opts){
    if (is.null(params$colors)){
        if (is_categorical_scale(opts)){
            ...
            params$colors <- c(d3_category10, d3_category10b)
        } else {
            params$colors <- c("steelblue", "brown")
        }
    }
    params$colors <- toJSON(params$colors)
    params$colors
}

get_color_by_param <- function(opts) {
    if (is.null(params$color_by)){
        params$color_by <- colnames(opts$data)[1]
    } else if (params$color_by %notin% colnames(opts$data)){
        stop("The input data doesn't contain a column named: ", params$color_by)
    }
...
get_domain_param <- function(opts){
    if (is.null(params$domain)){
        ...
        if (is.numeric(color_values)){
            params$domain <- range(color_values, na.rm = TRUE)
        } else {
            params$domain <- c(1, length(unique(color_values)))
        }
    }
    params$domain <- toJSON(params$domain)
