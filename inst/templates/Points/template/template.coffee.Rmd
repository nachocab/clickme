...
<html>
  ...
  <body>
    ...
    toggle_points = (category)->
        g_points.filter((d)-> d.group == category).classed("hide", ()->
            !d3.select(this).classed("hide")
        )
        # TODO: change the interaction so you can subselect categories one by one, but when you get to the last, they all show up (so there's no empty space)
        # TODO: give feedback as to which colors are selected/unselected
        g_points.filter((d)-> d.group != category).classed("hide", false)
            # .attr("display", (d)-> if d.attr())
