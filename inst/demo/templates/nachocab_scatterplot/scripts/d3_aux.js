...
(function() {
  ...
  this.append_main = function(opts) {
    ...
    main = d3.select(opts.selector).append(opts.element).attr('width', opts.width).attr('height', opts.height).style('background', opts.background).style('margin', opts.margin);
    if (opts.id) {
  ...
  this.append_plot = function(opts) {
    ...
    plot = plot.append("svg:g").attr("transform", "translate(" + opts.margin.left + "," + opts.margin.top + ")");
  ...
  this.draw_axes = function(plot, opts) {
    ...
    plot.axes = {};
    plot.axes.x = d3.svg.axis().scale(plot.scales.x).orient(opts.orientation_x);
    plot.axes.y = d3.svg.axis().scale(plot.scales.y).orient(opts.orientation_y);
    ...
    plot.selectAll(".axis line, .axis path").style("fill", "none").style("stroke", "black").style("shape-rendering", "crispEdges");
    return plot;
  };
  ...
  this.add_padding = function(scale, padding) {
    var range;
    if (padding == null) {
      padding = 20;
    }
    ...
    return scale.domain([range[0] - padding, range[1] + padding].map(scale.invert)).nice();
  };
