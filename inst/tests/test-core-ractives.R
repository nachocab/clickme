...
test_that("clickme_points reads the correct data input", {
    ...
    expect_equivalent(opts$data, data.frame(x = 2:4, y = 5:7, .name = LETTERS[1:3]))

    # x is a data frame (x, y, row names, extra column), no y
    opts <- clickme_points(data.frame(x = 2:4, y = 5:7, row.names = LETTERS[1:3], extra = 11:13), open = FALSE)
    expect_equivalent(opts$data, data.frame(x = 2:4, y = 5:7, .name = LETTERS[1:3]))
    ...
    expect_equivalent(opts$data, data.frame(x = 2:4, y = 5:7, .name = as.character(1:3)))
...
test_that("base::plot variable names are handled correctly", {
    ...
    expect_error(clickme_points(1:10, color_group = letters[1:10], color_domain = c(1,10), open = FALSE), "domain")
