...
test_that("doesn't overwrite an existing ractive, unless specified", {
    unlink(file.path(get_templates_path(), ractive), recursive = TRUE) # to be sure it doesn't exist

    new_ractive(ractive)
    expect_error(new_ractive(ractive), "ractive already exists")
    new_ractive(ractive, overwrite = TRUE)
    expect_true(file.exists(file.path(get_templates_path(), ractive)))

    unlink(file.path(get_templates_path(), ractive), recursive = TRUE)
})
