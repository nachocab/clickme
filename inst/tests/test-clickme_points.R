...
test_that("main is changed to title", {
    ...
    points <- Points$new(params = params)
...
test_that("color_domain is only used in quantitative scales", {
    ...
    expect_error(Points$new(params = params), "colorize has categorical values")
})
...
test_that("the palette has valid names", {
    ...
    expect_warning(Points$new(params = params), "palette names don't appear in colorize: d, e")

    params <- list(colorize = 1:5, palette = c(a = "blue", c = "green", d = "red", e = "yellow"))
    expect_error(Points$new(params = params), "an unnamed vector")
})
...
test_that("extra fields get added", {
    ...
    params <- list(extra = data.frame(extra1 = c(10,20,30), extra2 = c(100,200,300)))
    points <- Points$new(params = params)
    points$get_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]))
    expect_equivalent(points$data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))

    # extra can be a list
    params <- list(extra = list(extra1 = c(10,20,30), extra2 = c(100,200,300)))
    points <- Points$new(params = params)
    points$get_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]))
    expect_equivalent(points$data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))

    # extra can be a matrix
    params <- list(extra = cbind(extra1=c(10,20,30),extra2=c(100,200,300)))
    points <- Points$new(params = params)
    points$get_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]))
    expect_equivalent(points$data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))
...
test_that("limits reduce the size of the data", {
    ...
    points <- Points$new(params = params)
    points$get_data(1:10, 1:10)
    expect_equal(points$data$x, 2:8)

    params <- list(ylim = c(2,8))
    points <- Points$new(params = params)
    points$get_data(1:10, 1:10)
...
test_that("get_tooltip_content_points", {
    params <- list(xlab = "x", ylab = "y", extra = cbind(extra1=c(10,20,30), extra2=c(100,200,300)))
    points <- Points$new(params = params)
    points$get_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]))
    ...
    params <- list(xlab = "x", ylab = "y", colorize = c("a","a","b"), extra = cbind(extra1=c(10,20,30), extra2=c(100,200,300)))
    points <- Points$new(params = params)
    points$get_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]))
    tooltip_contents <- points$get_tooltip_content()
    expect_equal(tooltip_contents, "\"<strong>\" + d.point_name + \"</strong>\" + \"<br>\" + \"y: \" + format_property(d.y) + \"<br>\" + \"x: \" + format_property(d.x) + \"<br>\" + \"extra1: \" + format_property(d[\"extra1\"]) + \"<br>\" + \"extra2: \" + format_property(d[\"extra2\"])")
