...
test_that("the palette has valid names", {
    ...
    palette <- validate_points_params(params)$palette
...
test_that("point_names", {
    original_data <- xy_to_data(1:10, NULL)
    data <- get_points_data(original_data, list())
    expect_equal(data, data.frame(x = 1:10, y = 1:10, point_name = as.character(1:10)))

    data <- get_points_data(original_data, list(point_names = letters[1:10]))
...
params <- list(xlim = c(2,8))
    original_data <- xy_to_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]), NULL)

    # extra is a data frame
    data <- get_points_data(original_data, list(point_names = rownames(original_data), extra = data.frame(extra1 = c(10,20,30), extra2 = c(100,200,300))))
    expect_equivalent(data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))

    # extra is a list
    data <- get_points_data(original_data, list(point_names = LETTERS[1:3], extra = list(extra1 = c(10,20,30), extra2 = c(100,200,300))))
    expect_equivalent(data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))

    # extra is a matrix
    data <- get_points_data(original_data, list(point_names = LETTERS[1:3], extra = cbind(extra1=c(10,20,30),extra2=c(100,200,300))))
    expect_equivalent(data, data.frame(x = c("a", "b", "c"), y = 5:7, point_name = LETTERS[1:3], extra1 = c(10,20,30), extra2 = c(100, 200, 300)))
})

test_that("the palette determines the order in which the points are rendered", {
    original_data <- xy_to_data(1:5, NULL)
    params <- list(colorize = c(.5,.1,.2,.3,.4), palette = c("blue", "red", "green"))
    data <- get_points_data(original_data, params)
...
test_that("limits reduce the size of the data", {
    original_data <- xy_to_data(1:10, 1:10)
    params <- list(xlim = c(2,8))
    data <- get_points_data(original_data, params)
...
test_that("get_tooltip_content_points", {
    original_data <- xy_to_data(data.frame(x = c("a", "b", "c"), y = 5:7, row.names = LETTERS[1:3]), NULL)
    data <- get_points_data(original_data, list(point_names = rownames(original_data), extra = cbind(extra1=c(10,20,30),extra2=c(100,200,300))))
